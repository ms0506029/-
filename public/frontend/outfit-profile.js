...
async function loadProfile() {
  ...
  try {
    const data = await get('/api/outfits/profile.js', {
      handle: state.handle,
      page: 1,
...
async function loadLooks(page = 1, { append = false } = {}) {
  ...
  try {
    ...
    const data = await get('/api/outfits/looks.js', params);
...
async function handleInteraction(action, lookId, source = 'card') {
  ...
  try {
    let result;
    if (action === 'like') {
      result = await like(lookId);
    } else if (action === 'reference') {
      result = await markReference(lookId);
    } else if (action === 'purchase') {
      const firstItem = (look.items && look.items[0]) || {};
      result = await markPurchase(lookId, firstItem.variant_sku || firstItem.sku || null);
    }

    if (result?.metrics) {
      look.metrics = result.metrics;
    } else {
...
async function handleFollowToggle() {
 ...
 try {
    ...
    state.isFollowing = (result?.status || (nextState ? 'active' : 'unfollowed')) !== 'unfollowed';
    if (result?.stats) {
      state.profile.stats = result.stats;
    }
    if (nextState) {
      notify(result?.already ? '您已追蹤過此創作者' : '已追蹤創作者');
    } else {
      notify('已取消追蹤');
    }
    renderHero(state.profile);
...
async function openLookDetail(lookId) {
  ...
  if (!state.detailCache[lookId]) {
    ...
    try {
      const detail = await get('/api/outfits/look.js', { look_id: lookId });
      if (!detail?.look) {
